...
import { uploadReceipt } from '../services/dbService';
...
export const StaffPortal: React.FC<StaffPortalProps> = ({ claims, advances, currentUser, userId, onAddClaim, onAddAdvance }) => {
    ...
    const handleSubmitClaim = async (e: React.FormEvent) => {
        ...
        if (receiptFile) {
            try {
                const url = await uploadReceipt(receiptFile, userId);
                if (url) {
                    receiptUrl = url;
                } else {
                    alert("Warning: Receipt upload failed. Please check your internet connection or if the 'receipts' bucket exists.");
                }
            } catch (error) {
                console.error("Failed to upload file", error);
                alert("Error uploading receipt file.");
            }
        }
                    ...
                    <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden animate-in zoom-in-95 duration-200 max-h-[90vh] flex flex-col">
                        ...
                        <div className="p-6 overflow-y-auto custom-scrollbar">
                            <div className="mb-6">
                                ...
                                <input type="file" ref={cameraInputRef} className="hidden" accept="image/*,application/pdf" capture="environment" onChange={handleFileChange} />
