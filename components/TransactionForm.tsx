...
export const TransactionForm: React.FC<TransactionFormProps> = ({ onAddTransaction, onClose, userId }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (receiptFile) {
      try {
        const path = await uploadReceipt(receiptFile, userId);
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="pt-2 flex gap-3">
              ...
              <button
                ...
                className="flex-[2] px-6 py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 shadow-lg shadow-indigo-200 hover:shadow-indigo-300 hover:-translate-y-0.5 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isUploading ? (
